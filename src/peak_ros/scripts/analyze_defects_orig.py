...
# def find_peak_center_and_amplitude(points, n_top):
#     """
...
def process_point_cloud(points, dB):
    ...
    # center, center_amplitude = find_peak_center_and_amplitude(points, n_top=150)
    ...
    # Get amplitude at centre via nearest-neighbour (top 10, excluding NaN)
    distances = np.linalg.norm(points[:, :3] - center, axis=1)
    ...
    center_amplitude = np.sum(weights * nearest_amplitudes) / np.sum(weights)
    print(f"Amplitude at geometric centre (IDW, top 10 non-NaN): {center_amplitude:.6f}")
    ...
    if len(edge_radii) == 0:
        raise ValueError("No edge points found in any radial direction — check point cloud coverage and threshold.")
    ...
    return (filtered_points, center, center_amplitude, threshold,
            edge_u, edge_v, edge_radii, edge_angles,
            normal, plane_basis_1, plane_basis_2, filt_radii, centroid, diameter)
...
def main():
    ...
    # defects = [f'Defect_01_{Gain}Gain_{TVG}TVG', f'Defect_10_{Gain}Gain_{TVG}TVG']
...
Radial Edge:
  Mean radius:  {np.mean(filt_radii):.6f}
