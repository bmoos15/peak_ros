...
void PeakNodelet::populateBScanMessage(const peak_ros::Observation& obs_msg) {
    ...
    // FIRST PASS: Find max amplitude in gate region [depth_to_skip, max_depth - 10mm] across all
    // A-scans for normalization.  The upper bound is set 10mm below max_depth so the 6dB drop
    // method (threshold 0.505) centres on the defect area.
    float gate_norm_factor = (float)obs_msg.max_amplitude; // fallback to hardware max
    {
        const float gate_norm_offset = 0.010f; // 10 mm in metres
        float norm_max_amp = 0.0f;

        for (const auto& norm_ascan : obs_msg.ascans) {
            float pre_n = 1500.0f; // immersion mode is always active in the main loop
            int   pre_i = 0;
            int   pre_t = 0;
            bool  pre_found_fw  = false;
            float pre_depth_fw  = 0.0f;

            for (auto pre_amp_raw : norm_ascan.amplitudes) {
                float pre_z = (float)pre_i * pre_n * dt / 2.0f;
                if (pre_n != 1500.0f) {
                    pre_z = pre_z
                          + (float)pre_t * 1500.0f * dt / 2.0f
                          - (float)pre_t * (float)obs_msg.vel_material * dt / 2.0f;
                }

                // Mirror the TCG applied in the main loop
                float pre_amp = (float)pre_amp_raw;
                if (use_tcg_ && pre_z > 0.010f) {
                    pre_amp = pre_amp * std::pow(10.0f, (amp_factor_ * (pre_z / depth_factor_) / 20.0f));
                    float amp_limit = (float)obs_msg.max_amplitude * tcg_limit_;
                    if      (pre_amp >  amp_limit) pre_amp =  amp_limit;
                    else if (pre_amp < -amp_limit) pre_amp = -amp_limit;
                }

                // Detect front wall using ADC-range normalization so GANS affects the threshold
                float pre_amp_norm = pre_amp / adc_range;
                if (!pre_found_fw && pre_amp_norm > gate_front_wall_) {
                    pre_found_fw = true;
                    pre_depth_fw = pre_z;
                    pre_n = obs_msg.vel_material;
                    pre_t = pre_i;
                }

                if (pre_found_fw) {
                    float gate_start = pre_depth_fw + depth_to_skip_;
                    float gate_end   = pre_depth_fw + depth_to_skip_ + max_depth_ - gate_norm_offset;
                    if (pre_z > gate_start && pre_z < gate_end) {
                        float abs_amp = std::abs(pre_amp);
                        if (abs_amp > norm_max_amp) norm_max_amp = abs_amp;
                    }
                }
        ...
        if (norm_max_amp > 0.0f) {
            gate_norm_factor = norm_max_amp;
        }
    }
    ...
    for (const auto& ascan : obs_msg.ascans) {
        ...
        for (auto amplitude : ascan.amplitudes) {
            ...
            // Normalised to peak amplitude in gate region [depth_to_skip, max_depth - 10mm]
            normalised_amplitude = (float)amplitude / gate_norm_factor;
            // ADC-range normalisation used for front wall detection threshold so GANS has effect
            float hw_normalised_amplitude = normalised_amplitude * gate_norm_factor / adc_range;
            ...
            *bscan_iterAmps = (float)amplitude / (float)obs_msg.max_amplitude;
            ...
            if (!found_front_wall and
                     hw_normalised_amplitude > gate_front_wall_) {
