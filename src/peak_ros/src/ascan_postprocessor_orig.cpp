    ...
    void processObservation(const peak_ros::Observation& obs_msg, float frontwall_angle, float frontwall_depth) {
        ...
        for (const auto& ascan : obs_msg.ascans) {
            ...
            for (auto amplitude : ascan.amplitudes) {
                // Calculate depth using the EXACT same formula as peak_nodelet
                float z = (float)((float)i * (float)n * dt / 2.0f);
                
                // If in steel (immersion mode and velocity changed), adjust for water-steel transition
                if (immersion_ && n != 1500.0f){
                    ...
                    if (!velocity_switched && element_i == 0) {
                        ROS_INFO_ONCE("=== DEPTH ADJUSTMENT ACTIVE ===");
                        ROS_INFO_ONCE("Sample i=%d, t=%d, n=%.1f m/s", i, t, n);
                        ROS_INFO_ONCE("Water distance correction: %.6f m", (float)((float)t * 1500.0f * dt / 2.0f));
                        ROS_INFO_ONCE("Steel distance correction: %.6f m", (float)((float)t * (float)obs_msg.vel_material * dt / 2.0f));
                ...
                if (use_tcg_ && z > 0.0f) {
                    ...
                    normalised_amplitude = amplitude_tcg / (float)obs_msg.max_amplitude;
                } else {
